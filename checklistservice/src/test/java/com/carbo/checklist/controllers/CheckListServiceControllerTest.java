...
class CheckListServiceControllerTest {
    ...
    @Test
    void testGetCheckLists_EmptyJobId() throws Exception {
        mockMvc.perform(get("/v1/checklists/"))
                .andExpect(status().isBadRequest()); // Should throw BadRequest when jobId is missing
    }
    ...
    void testGetCheckList_NotFound() throws Exception {
        ...
        mockMvc.perform(get("/v1/checklists/{checkListId}", "1"))
                .andExpect(status().isInternalServerError()); // Should return 500 for empty Optional
    }

    @Test
    void testUpdateCheckList_HappyPath() throws Exception {
        HttpServletRequest request = new MockHttpServletRequest();
        String organizationId = "org1";
        when(request.getHeader("Organization-Id")).thenReturn(organizationId);
        ...
        verify(checkListService).updateCheckList(checkList);
    }

    @Test
    void testSaveCheckList_HappyPath() throws Exception {
        HttpServletRequest request = new MockHttpServletRequest();
        String organizationId = "org1";
        when(request.getHeader("Organization-Id")).thenReturn(organizationId);
        checkList.setOrganizationId(organizationId);
        ...
        verify(checkListService).saveCheckList(checkList);
    }

    @Test
    void testDeleteCheckList_HappyPath() throws Exception {
