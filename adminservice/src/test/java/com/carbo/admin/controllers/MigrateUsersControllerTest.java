...
class MigrateUsersControllerTest {
    ...
    void saveAiUsersAndCollectUnsaved_HappyPath() throws Exception {
        ...
        aiUserList.add(aiUser);

        when(migrateusersService.saveAiUsersAndCollectUnsaved(anyList())).thenReturn(new ArrayList<>());
        ...
        verify(migrateusersService).saveAiUsersAndCollectUnsaved(anyList());
    }
    ...
    void setUserAzureId_HappyPath() throws Exception {
        ...
        mockMvc.perform(put("/v1/user/migrate/setUserAzureId")
                .contentType(MediaType.APPLICATION_JSON)
                .content("[{\"emailAddress\":\"test@example.com\",\"azureUserId\":\"azureId123\"}]"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].azureId").value("azureId123"));

        verify(migrateusersService).setUserAzureId(anyList());
    }
