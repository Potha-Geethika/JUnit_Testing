...
class MigrateUsersControllerTest {
    ...
    void getAllUsersForAi_HappyPath() throws Exception {
        ...
        userResponseDTO.setOrganizationId("org123");
        userResponseDTO.setAzureId("azure123");
        List<UserResponseDTO> userList = Collections.singletonList(userResponseDTO);

        when(migrateusersService.getAllUsersForAi()).thenReturn(userList);
    ...
    void setUserAzureId_HappyPath() throws Exception {
        ...
        mockMvc.perform(put("/v1/user/migrate/setUserAzureId")
                .contentType(MediaType.APPLICATION_JSON)
                .content("[{\"emailAddress\":\"test@example.com\", \"azureUserId\":\"azure123\"}]"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].azureId").value("azure123"));
